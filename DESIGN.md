...
title: "Owen Wiki Template Kit Design"
status: adopted
product: "owen-wiki"
surface: "Markdown knowledge system viewed in Obsidian and generated outputs"
kind: document
updated: 2026-08-12
...
# Owen Wiki Template Kit Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | LLM agent와 함께 source를 수집·정제·연결·출력하는 지식 작업자 |
| Output job | Raw source를 provenance가 있는 wiki page와 ontology로 정제하고 목적별 output을 만든다 |
| Primary object | Markdown wiki page, source registry와 ontology relation |
| Primary action | Source를 ingest하거나 질문을 검증된 지식·output으로 승격한다 |
| Composition | Document-first Owen Graphite reading and operational reports |
| 핵심 판단 | Standalone app UI를 만들지 않고 Markdown·frontmatter·wikilink·SVG와 quality gate를 primary interface로 유지한다 |

## Context

owen-wiki는 LLM Wiki 구조와 scripts를 재사용 가능한 kit로 제공한다. Raw, wiki, outputs와 ontology의 ownership·provenance가 visual styling보다 우선한다.
...
- README.md
- UI-FOUNDATION-DESIGN-GUIDE.md
- templates/svg/
- scripts/wiki-quality-gates.py

## Foundation Selection

### Adopted

- Owen Graphite document hierarchy와 decision-first callout
- Markdown table과 structured references
- SVG a11y·budget·overflow contract
- Operational dashboard는 dense opaque surface

### Excluded Or App-Owned

- 문서 탐색을 별도 marketing site로 대체
- 근거 없는 metric·claim
- 모든 page의 decorative diagram
- Raw source 직접 수정

## Design Decisions

### D-01. Document is the primary UI

- **결정**: H1, summary/decision, evidence-near-claim, action, sources 순서를 유지한다
- **이유**: Obsidian, Git, PDF와 agent retrieval에서 같은 artifact를 사용하기 위해서다
- **검증**: Frontmatter, links, document quality gate를 실행한다

### D-02. Visual은 의미가 있을 때만 추가한다

- **결정**: 관계·순서·비교·수치 hierarchy가 text보다 명확할 때 SVG를 사용한다
- **이유**: 장식용 asset은 유지보수와 retrieval noise를 늘린다
- **검증**: SVG trigger, a11y, density, text overflow gate를 통과한다

### D-03. 운영 상태를 sidecar로 분리한다

- **결정**: 원문 Markdown과 machine-readable ledger·report를 분리하고 hash·path로 연결한다
- **이유**: 사람이 읽는 문서와 자동화 state가 서로 덮어쓰지 않게 한다
- **검증**: Quality gates와 sidecar rebuild를 실행한다
...
- python scripts/wiki-quality-gates.py
- ontology·link·tag·SVG gates
- Obsidian/Markdown/PDF visual review
